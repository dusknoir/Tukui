...
GameTooltip:HookScript("OnTooltipSetItem", function(self)
  if (IsAltKeyDown()) and (TukuiItemTooltip and not self.TukuiItemTooltip and (TukuiItemTooltip.id or TukuiItemTooltip.count)) then
    ...
    if left or right then
      self:AddDoubleLine(left or "", right or "")
    end

    if TukuiItemTooltip.id then
      local _, _, _, _, _, _, _, _, _, texture = GetItemInfo(link)
      self:AddLine("|cFFCA3C3CTexture|r " .. texture:sub(17))
    end
