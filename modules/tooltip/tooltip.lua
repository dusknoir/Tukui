...
local function UpdateTooltip(self)
  ...
  if self:GetAnchorType() == "ANCHOR_NONE" and TukuiTooltipAnchor then
    ...
    elseif point == "BOTTOMRIGHT" or point == "RIGHT" then
      ...
      elseif SkadaBarWindowSkada and SkadaBarWindowSkada:IsVisible() then
        self:ClearAllPoints()
        self:SetPoint("BOTTOMRIGHT", SkadaBarWindowSkada, "TOPRIGHT", 2, x)
      else
        self:ClearAllPoints()
...
local function GetColor(unit)
  if(UnitIsPlayer(unit) and not UnitHasVehicleUI(unit)) then
    ...
    if not color then return end -- sometime unit too far away return nil for color :(
    return Hex(color), color.r, color.g, color.b
  end
end
...
GameTooltip:HookScript("OnTooltipSetUnit", function(self)
  ...
  if(UnitIsPlayer(unit)) then
    ...
    elseif UnitIsDND(unit) then 
      self:AppendText(" |cffff0000(DND)|r")
    end

    local offset = 2
    if guild then
      GameTooltipTextLeft2:SetFormattedText("%s%s",
                IsInGuild() and GetGuildInfo("player") == guild and "|cff0090ff<"..guild..">|r" or "|cff00ff10<"..guild..">|r",
                rank and " (" .. rank .. ")" or "")
      offset = offset + 1
    end
  ...
  if UnitExists(unit.."target") and unit~="player" then
    ...
    --GameTooltip:AddLine(UnitName(unit.."target"), r, g, b)
        GameTooltip:AddLine("Target: " .. hex .. UnitName(unit.."target") .. "|r")
  end
